...
describe "/tstats/new.html.erb" do
  include TstatsHelper
  
  before(:each) do
    @tstat = mock_model(Tstat)
    @tstat.stub!(:new_record?).and_return(true)
  ...
  it "should render new form" do
    render "/tstats/new.html.erb"
    
    response.should have_tag("form[action=?][method=post]", tstats_path) do
